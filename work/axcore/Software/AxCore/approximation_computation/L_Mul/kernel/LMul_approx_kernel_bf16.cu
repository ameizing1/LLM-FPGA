...
__device__ __nv_bfloat16 LMul_approx_kernel_mul_bf16(__nv_bfloat16 a, __nv_bfloat16 b) {
...
__global__ void LMul_approx_kernel_bf16(
    ...
    if (row < M && col < N) {
        ...
        for (int k = 0; k < K; k++) {
            ...
            // sum += a * b;
            sum += LMul_approx_kernel_mul_bf16(a, b);
        }
        C[row * N + col] = sum;
    }
}
...
__global__ void LMul_approx_kernel_bf16_optimized(
        ...
        for (int i=0; i<STRIDE; i++) {
            for (int j=0; j<STRIDE; j++) {
                for (int k = 0; k < STEP; k++) {
                    ...
                    sum0[i][j] = __hadd(sum0[i][j], LMul_approx_kernel_mul_bf16(shared_A[tx + i][k], shared_B[ty + j][k]));
                    sum1[i][j] = __hadd(sum1[i][j], LMul_approx_kernel_mul_bf16(shared_A[tx + i][k], shared_B[ty + j + 32][k]));
                    sum2[i][j] = __hadd(sum2[i][j], LMul_approx_kernel_mul_bf16(shared_A[tx + i + 32][k], shared_B[ty + j][k]));
                    sum3[i][j] = __hadd(sum3[i][j], LMul_approx_kernel_mul_bf16(shared_A[tx + i + 32][k], shared_B[ty + j + 32][k]));
                }
            }
        }
        __syncthreads();
...
__global__ void LMul_approx_kernel_bf16_optimized_batched(
    const __nv_bfloat16* __restrict__ A,
    const __nv_bfloat16* __restrict__ B,
    __nv_bfloat16* __restrict__ C,
    const int M, const int N, const int K, 
    ...
    for (int t = 0; t < (K + TILE_SIZE - 1) / TILE_SIZE; t++) {
        ...
        for (int k = 0; k < TILE_SIZE; k++) {
            ...
            __nv_bfloat16 product_bf16 = LMul_approx_kernel_mul_bf16(a, b);
...
void launch_LMul_approx_kernel_bf16(__nv_bfloat16* A, __nv_bfloat16* B, __nv_bfloat16* C, int M, int N, int K) {
    ...
    LMul_approx_kernel_bf16_optimized<BLOCK, STRIDE><<<blocks_per_grid, threads_per_block>>>(A, B, C, M, N, K);
...
void launch_LMul_approx_kernel_bf16_batched(const __nv_bfloat16* A, const __nv_bfloat16* B, __nv_bfloat16* C, int M, int N, int K, int batch_size) {
    ...
    LMul_approx_kernel_bf16_optimized_batched<<<blocks_per_grid, threads_per_block>>>(A, B, C, M, N, K, batch_size);
...
__global__ void LMul_elementwisemul_single_kernel_bf16(
    const __nv_bfloat16* __restrict__ A, 
    const __nv_bfloat16* __restrict__ B, 
    __nv_bfloat16* __restrict__ C, 
    ...
    if (A_row_idx < A_row && A_col_idx < A_col) {
        ...
        C[A_row_idx * A_col + A_col_idx] = LMul_approx_kernel_mul_bf16(reg_A, reg_B);
    }
}

__global__ void LMul_elementwisemul_all_kernel_bf16(
    const __nv_bfloat16* __restrict__ A, 
    const __nv_bfloat16* __restrict__ B, 
    __nv_bfloat16* __restrict__ C, 
    int A_row, int A_col, int B_row, int B_col) {
...
__global__ void LMul_elementwisemul_rowwise_kernel_bf16(
    const __nv_bfloat16* __restrict__ A, 
    const __nv_bfloat16* __restrict__ B, 
    __nv_bfloat16* __restrict__ C, 
    int A_row, int A_col, int B_row, int B_col) {
...
void launch_LMul_elementwisemul_kernel_bf16(const __nv_bfloat16* A, const __nv_bfloat16* B, __nv_bfloat16* C, int A_row, int A_col, int B_row, int B_col) {
    dim3 threads_per_block(TILE_SIZE, TILE_SIZE);
    dim3 blocks_per_grid((A_col + threads_per_block.x - 1) / threads_per_block.x, (A_row + threads_per_block.y - 1) / threads_per_block.y);

    if (B_row == 1 && B_col == 1) {
        LMul_elementwisemul_single_kernel_bf16<<<blocks_per_grid, threads_per_block>>>(A, B, C, A_row, A_col, B_row, B_col);
    } else if (B_row == A_row && B_col == A_col) {
        LMul_elementwisemul_all_kernel_bf16<<<blocks_per_grid, threads_per_block>>>(A, B, C, A_row, A_col, B_row, B_col);
    } else if (B_row == A_row && B_col == 1) {
        LMul_elementwisemul_rowwise_kernel_bf16<<<blocks_per_grid, threads_per_block>>>(A, B, C, A_row, A_col, B_row, B_col);
    } else {
