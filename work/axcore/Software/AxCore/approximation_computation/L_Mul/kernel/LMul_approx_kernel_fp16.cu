...
__device__ half LMul_approx_kernel_mul_fp16(half a, half b) {
...
__device__ half LMul_approx_kernel_add_without_subnormal_rtz(half a, half b) {
    uint16_t a_bits = float16_to_uint16(a);
    uint16_t b_bits = float16_to_uint16(b);
...
__device__ half LMul_approx_kernel_add_without_subnormal_rtn(half a, half b) {
    uint16_t a_bits = float16_to_uint16(a);
    uint16_t b_bits = float16_to_uint16(b);
...
__device__ half LMul_approx_kernel_add_without_subnormal_efn(half a, half b) {
    uint16_t a_bits = float16_to_uint16(a);
    uint16_t b_bits = float16_to_uint16(b);
...
__device__ half LMul_approx_kernel_add_without_subnormal_tte(half a, half b) {
    uint16_t a_bits = float16_to_uint16(a);
    uint16_t b_bits = float16_to_uint16(b);
    # define shift_bits 3
    // if ((a_bits & 0x7FFF) == 0) {
...
__global__ void LMul_approx_kernel_fp16_optimized(
    half*  A, // A [M, K]
    half*  B, // B [N, K]
    half*  C,
    int M, int N, int K) {
        ...
        for (int i=0; i<STRIDE; i++) {
            for (int j=0; j<STRIDE; j++) {
                for (int k = 0; k < STEP; k++) {
                    ...
                    sum0[i][j] = __hadd(sum0[i][j], LMul_approx_kernel_mul_fp16(shared_A[tx + i][k], shared_B[ty + j][k]));
                    sum1[i][j] = __hadd(sum1[i][j], LMul_approx_kernel_mul_fp16(shared_A[tx + i][k], shared_B[ty + j + 32][k]));
                    sum2[i][j] = __hadd(sum2[i][j], LMul_approx_kernel_mul_fp16(shared_A[tx + i + 32][k], shared_B[ty + j][k]));
                    sum3[i][j] = __hadd(sum3[i][j], LMul_approx_kernel_mul_fp16(shared_A[tx + i + 32][k], shared_B[ty + j + 32][k]));
                }
            }
        }
        __syncthreads();
...
__global__ void LMul_approx_kernel_fp16_optimized_batched(
    ...
    for (int t = 0; t < (K + TILE_SIZE - 1) / TILE_SIZE; t++) {
        ...
        for (int k = 0; k < TILE_SIZE; k++) {
            ...
            half product_fp16 = LMul_approx_kernel_mul_fp16(a, b);
            // float product_fp32 = __half2float(product_fp16); 

            sum = __hadd(sum, product_fp16); // mse error 5.x
            // sum = LMul_approx_kernel_add_without_subnormal_rtz(sum, product_fp16); // mse error 7.x
            // sum = LMul_approx_kernel_add_without_subnormal_rtn(sum, product_fp16); // mse error 5.x
...
void launch_LMul_approx_kernel_fp16(half* A, half* B, half* C, int M, int N, int K) {
    ...
    LMul_approx_kernel_fp16_optimized<BLOCK, STRIDE><<<blocks_per_grid, threads_per_block>>>(A, B, C, M, N, K);
...
void launch_LMul_approx_kernel_fp16_batched(const half* A, const half* B, half* C, int M, int N, int K, int batch_size) {
    ...
    LMul_approx_kernel_fp16_optimized_batched<<<blocks_per_grid, threads_per_block>>>(A, B, C, M, N, K, batch_size);
...
__global__ void LMul_elementwisemul_single_kernel_fp16(
    const half* __restrict__ A, 
    const half* __restrict__ B, 
    half* __restrict__ C, 
    ...
    if (A_row_idx < A_row && A_col_idx < A_col) {
        ...
        C[A_row_idx * A_col + A_col_idx] = LMul_approx_kernel_mul_fp16(reg_A, reg_B);
    }
}

__global__ void LMul_elementwisemul_all_kernel_fp16(
    const half* __restrict__ A, 
    const half* __restrict__ B, 
    half* __restrict__ C, 
    int A_row, int A_col, int B_row, int B_col) {
...
__global__ void LMul_elementwisemul_rowwise_kernel_fp16(
    const half* __restrict__ A, 
    const half* __restrict__ B, 
    half* __restrict__ C, 
    int A_row, int A_col, int B_row, int B_col) {
...
void launch_LMul_elementwisemul_kernel_fp16(const half* A, const half* B, half* C, int A_row, int A_col, int B_row, int B_col) {
    dim3 threads_per_block(TILE_SIZE, TILE_SIZE);
    dim3 blocks_per_grid((A_col + threads_per_block.x - 1) / threads_per_block.x, (A_row + threads_per_block.y - 1) / threads_per_block.y);

    if (B_row == 1 && B_col == 1) {
        LMul_elementwisemul_single_kernel_fp16<<<blocks_per_grid, threads_per_block>>>(A, B, C, A_row, A_col, B_row, B_col);
    } else if (B_row == A_row && B_col == A_col) {
        LMul_elementwisemul_all_kernel_fp16<<<blocks_per_grid, threads_per_block>>>(A, B, C, A_row, A_col, B_row, B_col);
    } else if (B_row == A_row && B_col == 1) {
        LMul_elementwisemul_rowwise_kernel_fp16<<<blocks_per_grid, threads_per_block>>>(A, B, C, A_row, A_col, B_row, B_col);
    } else {
