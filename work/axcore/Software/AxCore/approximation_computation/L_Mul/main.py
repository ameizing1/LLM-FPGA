...
cuda_module = load(name="LMul_approx",
                   ...
                   sources=["kernel/LMul_approx.cpp", "kernel/LMul_approx_kernel_bf16.cu", "kernel/LMul_approx_kernel_fp16.cu"],
...
class LMulApproxFunctionBF16(torch.autograd.Function):
    ...
    def forward(ctx, A, B, C, M, N, K):
        cuda_module.torch_launch_LMul_approx_kernel_bf16(A, B, C, M, N, K)
        return C
...
class LMulApproxBF16(torch.nn.Module):
    def forward(self, A, B, C, M, N, K):
        return LMulApproxFunctionBF16.apply(A, B, C, M, N, K)
    
class LMulApproxFunctionFP16(torch.autograd.Function):
    @staticmethod
    def forward(ctx, A, B, C, M, N, K):
        cuda_module.torch_launch_LMul_approx_kernel_fp16(A, B, C, M, N, K)
        return C
...
class LMulApproxFP16(torch.nn.Module):
    def forward(self, A, B, C, M, N, K):
        return LMulApproxFunctionFP16.apply(A, B, C, M, N, K)
...
def main():
    ...
    # approx = LMulApproxBF16()
    approx = LMulApproxFP16()
    a[0, 0] = 0
    # b[:, 0] /= 10000000000
    ...
    print(f"LMul: {c_approx}, shape: {c_approx.shape}")
