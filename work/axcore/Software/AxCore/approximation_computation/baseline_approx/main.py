...
cuda_module = load(name="baseline_approx",
                   ...
                   sources=["/data-ssd/home/jiaxiang/codes/FPMA_TENSOR_CORE/approximation_computation/baseline_approx/kernel/baseline_approx.cpp", 
                            "/data-ssd/home/jiaxiang/codes/FPMA_TENSOR_CORE/approximation_computation/baseline_approx/kernel/baseline_approx_kernel_bf16.cu",
                            "/data-ssd/home/jiaxiang/codes/FPMA_TENSOR_CORE/approximation_computation/baseline_approx/kernel/baseline_approx_kernel_fp16.cu"],
...
class BaselineApproxFunctionBF16(torch.autograd.Function):
    ...
    def forward(ctx, A, B, C, M, N, K):
        cuda_module.torch_launch_baseline_approx_kernel_bf16(A, B, C, M, N, K)
        return C
...
class BaselineApproxBF16(torch.nn.Module):
    def forward(self, A, B, C, M, N, K):
        return BaselineApproxFunctionBF16.apply(A, B, C, M, N, K)

class BaselineApproxFunctionFP16(torch.autograd.Function):
    @staticmethod
    def forward(ctx, A, B, C, M, N, K):
        cuda_module.torch_launch_baseline_approx_kernel_fp16(A, B, C, M, N, K)
        return C
...
class BaselineApproxFP16(torch.nn.Module):
    def forward(self, A, B, C, M, N, K):
        return BaselineApproxFunctionFP16.apply(A, B, C, M, N, K)
...
def main():
    ...
    print(f"Baseline: {c_approx}, shape: {c_approx.shape}")
