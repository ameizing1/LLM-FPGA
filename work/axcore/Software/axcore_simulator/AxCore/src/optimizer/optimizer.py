...
# axcore,1000000000,1,1,4,4, 1 ,3005,235187,152352
# figlut,1000000000,1,1,4,4, 1 ,3005,356239,152352
# figna,1000000000,1,1,4,4, 1 ,3005,246415,152352
# fpma,1000000000,1,1,4,4, 1 ,6219,814664,152352
# fpe,1000000000,1,1,4,4, 1 ,26447,2168456,152352

# axcore,1000000000,1,1,4,4, 1 ,3005,262125,152352
# figlut,1000000000,1,1,4,4, 1 ,5793,720103,152352
# figna,1000000000,1,1,4,4, 1 ,4523,378176,152352
# fpma,1000000000,1,1,4,4, 1 ,6219,814664,152352
# fpe,1000000000,1,1,4,4, 1 ,26447,2168456,152352
...
def get_stats_fast(conv_params, tiling, order_type, verbose=False):
    ...
    writes['act'] = ow * oh * \
            ceil_a_by_b(K * K * ic, acc_obj.M) * acc_obj.M * \
            b * iprec
    ...
    if weight_stationary:
        ...
        stats.reads['act'] += num_tiles / 64 * (kw * kh * ic * oc) * (b * ow * oh) * iprec
        stats.reads['out'] += num_tiles / 64 * (kw * kh * ic * oc) * (b * ow * oh) * oprec
        stats.writes['out'] += num_tiles / 64 * (kw * kh * ic * oc) * (b * ow * oh) * oprec
        # stats.reads['out'] += (kw * kh * ic * oc) * (b * ow * oh) * oprec
        # stats.writes['out'] += (kw * kh * ic * oc) * (b * ow * oh) * oprec
        # stats.reads['wgt'] += num_tiles * (kw * kh * ic * oc) * wprec
        stats.reads['wgt'] += num_ic * num_oc * (kw * kh * ic * oc) * wprec
    else:
        if verbose:
...
def optimize_for_order(conv_params):
    ...
    order = {tuple(loops)} #set(permutations(loops))
