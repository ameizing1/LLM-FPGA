...
# from AxCore.src.simulator.simulator import Simulator
from AxCore.src.utils.utils import lookup_pandas_dataframe
                                    ...
                                    for obuf in list_obuf:
                                        for bw in list_bw:
                                            ...
                                            for b in list_bench:
                                                ...
                                                if bench_type == 'axcore':
                                                    nn = benchmarks.get_bench_nn_axcore(b, batch_size)
                                                elif bench_type == 'ant':
                                                    nn = benchmarks.get_bench_nn_ant(b, batch_size)
                                                elif bench_type == 'ant_weight':
...
def check_pandas_or_run_ax(sim, dataframe, sim_sweep_csv, batch_size=1, config_file='./conf.ini', list_bench=None, bench_type='ant', weight_stationary = False):
    ld = {}
